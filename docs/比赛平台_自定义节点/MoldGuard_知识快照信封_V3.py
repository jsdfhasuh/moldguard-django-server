...
class MoldGuardKnowledgeSnapshotEnvelopeV3(Component):
    display_name = "MoldGuard 知识快照信封 V3（单输出）"
    description = "接收平台模块化提取结果，补齐内部字段并输出 Django 知识快照请求。"
    icon = "database"
    name = "MoldGuardKnowledgeSnapshotEnvelopeV3"
    ...
    inputs = [
        ...
        HandleInput(
            ...
            input_types=["Data", "Message"],
            info=(
                "连接模块化提取的 Data/Message；每项只需 title、content、source，"
                '也兼容 JSON Message 和 {"results": [...]} 包装。'
            ),
            required=False,
        ),
        MessageTextInput(
    ...
    def _decode_payload(cls, value: Any, source_label: str) -> Any:
        if isinstance(value, Data):
            ...
            raw = value.data if isinstance(value.data, (dict, list)) and value.data else value.text
        else:
            raw = value
