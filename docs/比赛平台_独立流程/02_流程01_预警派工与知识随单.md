...
1. 在自定义组件中确认已保存四个单输出组件：`请求信封 V3`、`单输入 HTTP V2`、`响应信封 V3`、`知识快照信封 V4`。知识快照 V4 必须作为新组件注册，不要覆盖原来的 V2/V3。
...
节点 17C 是本段唯一一次大模型调用。解析器只在大模型之前把知识库 `list[Data]` 合并成文本；大模型之后不需要再接“解析器”“消息转数据”或“结构化输出”。知识快照 V4 会优先使用合法的结构化 `Message.data.results`，否则读取 `Message.text`，并兼容 `Message.data.text`、`Data.data.text` 等运行包装。它不会把自由文本猜成三字段对象，也不会修复非法或被截断的 JSON。
...
节点 18 的 `knowledge_results` 输入在知识快照 V4 上仍显示为“模块化提取结果”，这是兼容历史端口名，不代表必须使用模块化提取节点。直接连接 `17C.response` 即可。
...
| 可见 JSON 有三字段，但知识快照仍报告三字段全部缺失 | 仍在使用 V3，组件误把大模型 `Message.data` 运行包装当成知识项 | 新注册知识快照 V4 并替换节点 18；V4 会读取 `Message.text`/`data.text`，失败时查看异常末尾的安全输入诊断 |
...
| 节点 18 找不到 `knowledge_results` | 画布显示的是兼容历史名称“模块化提取结果” | 将 `17C.response` 连接到该端口；不要增加后置解析器 |
