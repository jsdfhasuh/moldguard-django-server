...
| MoldGuard 请求信封 V2 | 演示批次 `demo_run_id` 或上游成功信封 `upstream` | 请求信封 `request` |
| MoldGuard 单输入 HTTP V2 | 请求信封 `request` | HTTP 响应信封 `response` |
| MoldGuard 响应信封 V2 | HTTP 响应信封 `response` | 业务响应信封 `result` |
...
1. 在自定义组件中确认已保存四个 V2 组件：`请求信封 V2`、`单输入 HTTP V2`、`响应信封 V2`、`知识快照信封 V2`。
2. 把 31 个节点先放到画布上，并按“节点清单”重命名。
3. 每个“请求信封 V2”都要先选“请求类型”并保存，对应的 `demo_run_id` 或 `upstream` 输入口才会显示。
...
| 节点 | 固定配置 | 左侧输入 | 右侧输出去向 |
|---|---|---|---|
| 01 聊天输入 | 测试文本 `DEMO-20260814-001`；文件留空 | 无 | `message -> 02.demo_run_id` |
| 02 扫描请求信封 | 请求类型=`扫描预警`；后端基础地址=`https://moldguard.oracle.19970219.xyz`；本 V2 节点不显示模具 ID 字段 | `demo_run_id <- 01.message` | `request -> 03.request` |
...
```text
15.true_result -> 18.upstream             # 传工单、人员和演示批次
...
节点 26 从 `21.true_result.data.context` 取得工单 ID 和演示批次，生成：
...
|---:|---|---|---|
| 1 | `01.message`（消息） | `02.demo_run_id`（演示批次） | 本次运行的稳定幂等 ID |
...
| 7 | `05.true_result`（真） | `07.upstream`（上游成功信封） | 将工单 ID 和演示批次传给派工请求 |
...
| 20 | `15.true_result` | `18.upstream` | 把工单、派工人、演示批次传给知识快照信封 |
...
|---|---|
| 04 扫描响应 | `demo_run_id`、`scan_scope`、`scanned_count`、`triggered_count`、`triggered_work_order_ids`、`selected_mold_id`、`work_order_id`、`alert_id`、`status`、`scan_code` |
...
| 自定义节点仍显示多个可选输出 | 使用了旧版组件 | 确认节点名称中包含 `V2`，并且只有一个右侧输出端口 |
...
4. 节点 02 的请求体不含 `mold_ids`，Django 返回全部非禁用模具的扫描结果。
