...
- 预期连线数：36
- 输入示例：`开始检查`
- 系统运行批次：节点 02 自动生成 `FLOW01-<北京时间戳>`，用户无需填写
...
```mermaid
flowchart LR
    A["01 输入检查指令"] --> B["02 扫描请求信封"] --> C["03 扫描 HTTP"] --> D["04 扫描响应信封"] --> E{"05 路由扫描"}
    E -->|假| F["06 扫描失败"]
...
| MoldGuard 请求信封 V3 | 启动指令 `demo_run_id`（内部兼容名）或上游成功信封 `upstream` | 请求信封 `request` |
...
| 提示词 | 保存模板后出现动态端口 `maintenance_context` | 提示消息 `prompt` |
| 大模型 | 输入 `input_value` | 响应 `response` |

## 连线前必须先做

1. 在自定义组件中确认已保存四个单输出组件：`请求信封 V3`、`单输入 HTTP V2`、`响应信封 V3`、`知识快照信封 V2`。
2. 把 31 个节点先放到画布上，并按“节点清单”重命名。
3. 每个“请求信封 V3”都要先选“请求类型”并保存，对应的“启动指令”（内部名 `demo_run_id`）或 `upstream` 输入口才会显示。
4. 先把节点 23 的提示词模板保存，再连 `maintenance_context`；动态端口在模板保存前不会出现。
5. 流程 01 不使用旧版 `MoldGuard 请求适配器`、`MoldGuard 响应适配器` 或原生 `JSON HTTP 请求`。如果 HTTP 节点还要求单独的 URL 和 Body，说明节点选错了。
...
| 节点 | 固定配置 | 左侧输入 | 右侧输出去向 |
|---|---|---|---|
| 01 聊天输入 | 测试文本 `开始检查`；文件留空 | 无 | `message -> 02.demo_run_id`（画布显示“启动指令”） |
| 02 扫描请求信封 | 请求类型=`扫描预警`；后端基础地址=`https://moldguard.oracle.19970219.xyz`；不显示模具 ID 或批次 ID 输入 | `demo_run_id <- 01.message` | `request -> 03.request` |
...
节点 02 会自动生成 `POST /api/v1/alerts/scan`，请求体只有：

```json
{"client_request_id":"FLOW01-<北京时间戳>-scan"}
```

节点 02 只把聊天文本作为启动指令，同时按北京时间生成一次带微秒的系统运行批次，例如 `FLOW01-20260814-153045-123456`。该值写入 `context.demo_run_id`，供本次运行后续请求稳定复用；用户不需要也不能手工输入批次 ID。
...
响应信封会把全部触发工单 ID 保存到 `context.triggered_work_order_ids`，并选择第一个有效触发工单继续当前单输出流程。若所有模具都未到保养阈值，会走 `05.false_result -> 06`，这是正常业务结果。
...
```json
{"client_request_id":"FLOW01-<同一北京时间戳>-send-email"}
```
...
|---:|---|---|---|
| 1 | `01.message`（消息） | `02.demo_run_id`（画布显示“启动指令”） | “开始检查”等启动文本；节点 02 自行生成系统运行批次 |
| 2 | `02.request`（请求信封） | `03.request`（请求信封） | 扫描的方法、URL、Body 和初始 context |
| 3 | `03.response`（HTTP 响应信封） | `04.response`（HTTP 响应信封） | 扫描 HTTP 状态、Body 和 context |
| 4 | `04.result`（业务响应信封） | `05.input_text`（文本输入） | 让条件节点查找 `[MOLDGUARD_OK]` |
| 5 | `04.result`（业务响应信封） | `05.message`（消息） | 让条件节点原样传递完整 Message.data |
| 6 | `05.false_result`（假） | `06.input_value`（文本） | 展示未到期、校验失败或 HTTP 失败 |
| 7 | `05.true_result`（真） | `07.upstream`（上游成功信封） | 将工单 ID 和系统运行批次传给派工请求 |
| 8 | `07.request` | `08.request` | 自动派工请求信封 |
| 9 | `08.response` | `09.response` | 自动派工 HTTP 响应信封 |
| 10 | `09.result` | `10.input_text` | 判断派工响应是否含成功标记 |
| 11 | `09.result` | `10.message` | 在真/假分支中传递派工 Message.data |
| 12 | `10.false_result` | `11.input_value` | 展示派工失败原因 |
| 13 | `10.true_result` | `12.upstream` | 将工单和 `employee_id` 传给知识上下文请求 |
| 14 | `12.request` | `13.request` | 知识上下文 GET 请求信封 |
| 15 | `13.response` | `14.response` | 知识上下文 HTTP 响应信封 |
| 16 | `14.result` | `15.input_text` | 判断知识上下文是否成功 |
| 17 | `14.result` | `15.message` | 传递搜索词与完整 context |
| 18 | `15.false_result` | `16.input_value` | 展示知识上下文失败 |
| 19 | `15.true_result` | `17.search_query` | 把生成的模具保养搜索词传给平台知识库 |
| 20 | `15.true_result` | `18.upstream` | 把工单、派工人、系统运行批次传给知识快照信封 |
| 21 | `17.search_results` | `18.knowledge_results` | 把知识库返回的 `list[Data]` 传给快照校验器 |
| 22 | `18.request` | `19.request` | 知识快照 POST 请求信封 |
| 23 | `19.response` | `20.response` | 知识回写 HTTP 响应信封 |
| 24 | `20.result` | `21.input_text` | 判断知识快照是否保存成功 |
| 25 | `20.result` | `21.message` | 传递已保存知识文本与完整 context |
| 26 | `21.false_result` | `22.input_value` | 展示知识快照回写失败 |
| 27 | `21.true_result` | `23.maintenance_context` | 把工单、人员和检索知识交给 AI 任务卡模板 |
| 28 | `23.prompt` | `24.input_value` | 组装好的任务卡提示消息 |
| 29 | `24.response` | `25.input_value` | AI 任务卡预览正文 |
| 30 | `21.true_result` | `26.upstream` | 在知识快照成功后独立启动 Django 发信分支 |
| 31 | `26.request` | `27.request` | Django 发信 POST 请求信封 |
| 32 | `27.response` | `28.response` | Django 发信 HTTP 响应信封 |
| 33 | `28.result` | `29.input_text` | 判断是否真实返回 `SENT + email_message_id` |
| 34 | `28.result` | `29.message` | 在发信真/假分支中传递完整响应 |
| 35 | `29.true_result` | `30.input_value` | 展示 Django 真实发信成功 |
| 36 | `29.false_result` | `31.input_value` | 展示发信失败或响应字段不完整 |
...
|---|---|
| 04 扫描响应 | `demo_run_id`（系统生成）、`start_command`、`scan_scope`、`scanned_count`、`triggered_count`、`triggered_work_order_ids`、`selected_mold_id`、`work_order_id`、`alert_id`、`status`、`scan_code` |
...
2. 31 个节点和 36 条连线均已完成；7 个一拖二位置与本文一致。
3. 五个“如果-否则”节点都同时连了 `input_text` 和 `message`。
4. 用户只需输入“开始检查”等指令；节点 02 自动生成 `FLOW01-<北京时间戳>-scan`，请求体不含 `mold_ids`，Django 返回全部非禁用模具的扫描结果。
